...
const EMPTY_BUDGET = {
  estimatedBudget:"", venueExpense:"", foodExpense:"",
  decorExpense:"", printingExpense:"", otherExpense:"", budgetNotes:"",
};
...
export default function SubmitBudget({ onLogout }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.estimatedBudget) { setError("Total budget is required."); return; }
    ...
    <DashboardLayout onLogout={onLogout}>
      ...
      {event?.status === "FACULTY_APPROVED" && (
        <div className="max-w-2xl mb-6 flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
          ...
          <div>
            <p className="text-sm font-semibold text-emerald-700">Faculty Advisor approved your event!</p>
            <p className="text-xs text-emerald-600">Submit your budget below. It will be reviewed by the SDW Coordinator.</p>
          </div>
        </div>
      )}
      ...
      <div className="max-w-2xl">
        <form onSubmit={handleSubmit} className="glass-card p-8 flex flex-col gap-5">
          ...
          <div>
            <label className="field-label">Total Estimated Budget (₹) <span className="text-red-500">*</span></label>
            <div className="relative">
              <span className="absolute left-3.5 top-2.5 text-slate-400 text-sm">₹</span>
              <input name="estimatedBudget" type="number" min="0" value={form.estimatedBudget}
                onChange={set} placeholder="0" required className="input pl-8"/>
            </div>
          </div>
              ...
              {[
                ...
                <div key={name}>
                  ...
                  <div className="relative">
                    ...
                    <input name={name} type="number" min="0" value={form[name]}
                      onChange={set} placeholder="0" className="input pl-7 text-sm"/>
                  </div>
                </div>
              ))}
          ...
          <div>
            ...
            <textarea name="budgetNotes" value={form.budgetNotes} onChange={set}
              rows={3} placeholder="Explain how the budget will be used, any sponsorships, etc."
              className="input resize-none"/>
          </div>

          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={loading || event?.status !== "FACULTY_APPROVED"} className="btn flex-1 py-2.5">
